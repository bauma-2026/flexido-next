...
/**
 * FLEX 7 CNC Slim-only layout. Extracted from the shared `SystemPageTemplate`
 * (still used by FLEX 25-50 only) so this clarity pass can't touch FLEX.
 * Mirrors the primitives already validated on TMX/IMM: product identity
 * separated from its own technical-data band, closed-matrix Applications
 * (no numbering, no `divide-x`), a flattened equal-peer documented-options
 * grid, a minimal benefits pair, and the card + proof-link related-solution
 * pattern. CNC has no `detailPanel`/`gallery` content, so no split-statement
 * or image-gallery section is rendered — those primitives aren't forced
 * where CNC's content doesn't call for them. Consumes the same
 * `SystemPageContent` data as before.
 */

/**
 * Technical-data band eyebrow — mirrors TMX/IMM's local-only label (no
 * existing equivalent lives in `content/systems` or `technicalLabels.ts`,
 * both shared with FLEX 25-50, which still keeps its spec table inside the
...
export default function CncPageTemplate({ locale, content }: { locale: Locale; content: SystemPageContent }) {
  ...
  const [applicationsGrid] = content.itemGrids ?? [];
  const optionsGrid = content.optionGrids?.[0];
  ...
  return (
    <>
      ...
      <main className="overflow-x-hidden bg-white text-neutral-950">
        ...
        {/* Intro — unchanged text-left/image-right composition, same grammar as TMX/IMM/FLEX */}
        <Section>
          <Container>
        ...
        {/* FLEX 7 CNC Slim product identity — image + eyebrow/title/description + the quiet highlights meta layer only. Reordered so identity text renders before the image on mobile (same trick as TMX/IMM): intro image → identity text → product image, never two images back to back. Technical data moved out into its own band directly below, so it no longer reads as leftover copy hanging under the image. */}
        {system ? (
        ...
        {/* Technical data band — separate formal-specification layer directly after product identity, before Applications: own top rule, own compact (tight) padding, width chosen for CNC's actual 4-row values (same 520px measure already proven for this exact value set on TMX). */}
        ...
        {/* Documented loading/unloading systems — peer set, not a sequence: closed matrix (outer border, explicit dividers) matching TMX/IMM Applications grammar, no numbering. Explicit border classes instead of `divide-x` — that utility has been unreliable in this project (computing to 0px) — full container width so the three peer items get the same room instead of being capped under the heading's measure. */}
        {applicationsGrid ? (
        ...
        {/* Additional stations — four documented options at equal peer weight, no primary/secondary hierarchy, no numbering. Source renders (`/raw/cnc-option-*.webp`) are a normalized set, each exactly 4:3 (1448×1086) — matching the frame exactly, so object-cover and object-contain render pixel-identical with zero cropping; object-cover is the simpler, standard choice used elsewhere on the site. Borderless, no bg fill — same clean family as FLEX's own multi-group tiles. */}
        {optionsGrid ? (
          <Section className="bg-white">
            <Container>
              <div className="max-w-[760px]">
                <p className="text-[11px] uppercase tracking-[0.18em] text-neutral-400">{optionsGrid.eyebrow}</p>
                <h2 className="mt-4 text-[32px] font-semibold leading-tight tracking-[-0.04em] text-neutral-950 sm:text-[44px]">
                  {optionsGrid.heading}
                </h2>
              </div>
              <div className="mt-8 grid grid-cols-2 gap-x-4 gap-y-6 lg:grid-cols-4 lg:gap-5">
                {optionsGrid.items.map((item) => (
        ...
        {/* Benefits — a minimal 2-item pair, not an overbuilt matrix: no heading, no border box, lower weight than Applications, same calm typography IMM already uses for this exact role. */}
        {content.benefits ? (
        ...
        {/* Related solution / continuation — same navigation-card pattern validated on IMM (bordered/radius primary card; a tagged proof item renders as a subordinate text link below it, not a second card), so this reads as "where to go next" rather than more explanation. */}
        ...
        {/* CTA — dark image-backed band, matching Middleware/IMM/TMX/FLEX 25-50 */}
